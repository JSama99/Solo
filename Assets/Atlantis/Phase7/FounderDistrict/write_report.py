...
A=json.loads((R/'build_audit.json').read_text());V=json.loads((R/'verification.json').read_text());assert all(V['checks'].values())
slot=(-875,-1030);boundary=math.dist(slot,(-720,-540));core=math.dist(slot,(-460,-340));spire=math.dist(slot,(50,420));main=next(p for p in A['paths'] if p['name']=='Founder_ToStartup_Walk');length=sum(math.dist(a,b) for a,b in zip(main['points'],main['points'][1:]));types=collections.Counter(s['typology'] for s in A['sites'])
metrics='\n'.join(f"| {k.replace('_',' ')} | {V['phase6_metrics'][k]:,} | {V['phase7_metrics'][k]:,} | {V['phase7_metrics'][k]-V['phase6_metrics'][k]:+,} |" for k in V['phase6_metrics'])
files=sorted(str(p.relative_to(Path.cwd())) for p in R.rglob('*') if p.is_file())
text=f'''# Atlantis Phase 7 — Founder District
...
| Typology | Count |
| --- | --- |
'''+ '\n'.join(f'| {k} | {n} |' for k,n in types.items())+f'''
...
Materials are shared siding, cream trim, brick, concrete paving, asphalt, restrained glazing, wood, metal, lawn, leaf and warm window surfaces. Seven building assemblies are preserved in the hidden Founder_KitLibrary. {A['authored_building_components']:,} authored building components are consolidated into architecture and separate lighting-surface meshes per building. {A['reusable_dimension_material_meshes']} shared dimension/material box modules support windows, trim, mullions, doors, porches, roof panels, service boxes, fences and street furniture. Gable geometry and one shared low-poly canopy mesh supplement these modules. Consolidation reduces objects; this is not runtime batching or measured RealityKit optimization.
...
| Founder shared box modules | 0 | {A['reusable_dimension_material_meshes']} | +{A['reusable_dimension_material_meshes']} |
...
'''+ '\n'.join('- '+n for n in A['review_cameras'])+'''
...
'''+ '\n'.join('- '+p for p in files if not p.endswith(('inspect.py','prior_state.json')))+'''
...
'''
Path('Documentation/Atlantis/PHASE7_FOUNDER_DISTRICT_REPORT.md').write_text(text)

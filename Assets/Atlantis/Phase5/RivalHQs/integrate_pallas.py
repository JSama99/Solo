...
R=os.path.dirname(os.path.abspath(__file__));A=R+'/PallasAI'
def sig(o):
...
bpy.ops.wm.open_mainfile(filepath=A+'/Blender/PallasAI_Integration_Candidate.blend');s=bpy.context.scene;checks=json.load(open(A+'/export_verification.json'))['checks'];checks['original_objects_preserved']=all(bpy.data.objects.get(n) and sig(bpy.data.objects[n])==h for n,h in base.items())
...
for n in ['Founder_To_City','TheSpire_StartupSightline','Venture_Pallas','Media_RivalSkyline','TechCore_Rivals_Aerial','UnicornHeights_View']:
